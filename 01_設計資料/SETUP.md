...
1つのターミナルで両方のサービスを起動します。プロセス監視とログ管理が自動化されています。
...
停止する場合: `Ctrl + C`（両方のサーバーがグレースフルシャットダウン）

**利点**:
- 1つのターミナルで完結
- プロセス監視（異常終了を自動検知）
- ログファイルに出力が保存される（`.dev/backend.log`, `.dev/frontend.log`）
- グレースフルシャットダウン保証

詳細は [開発ツールドキュメント](../98_ツール/README.md) を参照してください。
...
- または別のターミナルから: `./doc/98_ツール/stop-dev.sh`
